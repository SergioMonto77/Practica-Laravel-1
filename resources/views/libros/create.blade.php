...
@section('contenido')
    ...
    <form name="as" method="POST" action="{{route('libros.store')}}" class="hidden bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-1/2 mx-auto">

        @csrf

        <div class="mb-6">
            <label for="t" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Título</label>
            <input name="titulo" value="{{old('titulo')}}" type="text" id="t" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Título">
            @error('titulo')
        ...
        <div class="mb-6">
            ...
            <input name="resumen" value="{{old('resumen')}}" type="text" id="r" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Resumen">
            @error('resumen')
                <p class="mt-2 text-red-800 text-sm">***{{$message}}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input name="pvp" value="{{old('pvp')}}" type="number" step="0.1" id="p" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Precio">
            @error('pvp')
                <p class="mt-2 text-red-800 text-sm">***{{$message}}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <input name="stock" value="{{old('stock')}}" type="number" step="1" id="s" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Stock">
            @error('stock')
                <p class="mt-2 text-red-800 text-sm">***{{$message}}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <select name="user_id" id="u" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                @foreach ($users as $k=>$v)
                    <option value="{{$k}}" @selected($k==old('user_id'))>{{$v}}</option> {{--muestro los nombres pero guardo los id (gracias a ->pluck('clave', 'valor'))--}}
    ...
    <x-form name="ass" action="{{route('libros.store')}}" class="bg-gray shadow-md rounded px-8 pt-6 pb-8 mb-4 w-1/2 mx-auto">

        <x-form-input name="titulo" type="text" label="Título" />
        <x-form-input name="resumen" type="text" label="Resumen" />
        <x-form-input name="pvp" type="number" step="0.1" label="Precio (€)" />
